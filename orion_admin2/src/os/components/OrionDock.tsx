...
export function OrionDock() {
  ...
  // Real OS-style auto-hide: the Dock is hidden by default and is revealed
  // whenever the pointer reaches the bottom edge of the display.  The old
  // implementation had no auto-hide state at all, so CSS/settings could not
  // make it work.
  ...
  const scheduleDockHide = useCallback(() => {
    ...
    hideTimerRef.current = window.setTimeout(() => {
      ...
      if (!el?.matches(':hover')) setDockVisible(false);
    }, 900);
  }, [clearDockHideTimer]);

  useEffect(() => {
    const onPointerMove = (e: PointerEvent) => {
      const edge = Math.max(18, Math.min(32, Math.round(window.innerHeight * 0.018)));
      if (e.clientY >= window.innerHeight - edge) {
        clearDockHideTimer();
        setDockVisible(true);
      } else if (dockVisible && !dockRef.current?.matches(':hover')) {
        scheduleDockHide();
      }
    };
    ...
    const initial = window.setTimeout(() => setDockVisible(false), 1200);
  ...
  }, [clearDockHideTimer, scheduleDockHide, dockVisible]);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        dockVisible ? "translate-y-0 pointer-events-auto" : "translate-y-[calc(100%+28px)] pointer-events-none"
      )}
      onContextMenu={handleDockSurfaceContextMenu}
      onMouseEnter={() => { clearDockHideTimer(); setDockVisible(true); }}
