...
export const BUILT_IN_LOGO_PATH = '/orion-9-logo-transparent-v2.png';
export const LEGACY_BUILT_IN_LOGO_PATH = '/orion-9-logo.png';
export const PREVIOUS_TRANSPARENT_LOGO_PATH = '/orion-9-logo-transparent.png';
...
export const defaultBranding: BrandingConfig = {
  ...
  logoUrl: BUILT_IN_LOGO_PATH,
  logo: BUILT_IN_LOGO_PATH,
  faviconUrl: null,
...
export function normalizeBranding(raw: any): BrandingConfig {
  ...
  const osName = safeStr(raw.osName || raw.appName || raw.productName || raw.applicationName, defaultBranding.osName);
  const productName = safeStr(raw.productName || raw.applicationName || raw.appName || raw.osName, defaultBranding.productName);
  const appName = safeStr(raw.appName || productName || osName, defaultBranding.appName);
  const applicationName = safeStr(raw.applicationName || appName || productName, defaultBranding.applicationName);
  ...
  const rawLogo = raw.logoUrl !== undefined ? raw.logoUrl : (raw.logo !== undefined ? raw.logo : null);
  const normalizedRawLogo = typeof rawLogo === 'string' ? rawLogo.trim() : '';
  // Migrate the built-in opaque legacy asset automatically. Custom uploaded
  // logos and external logo URLs are left untouched.
  const logo = (normalizedRawLogo === LEGACY_BUILT_IN_LOGO_PATH || normalizedRawLogo === PREVIOUS_TRANSPARENT_LOGO_PATH)
    ? BUILT_IN_LOGO_PATH
    : (normalizedRawLogo && normalizedRawLogo !== '[object Object]' && normalizedRawLogo !== 'null' && normalizedRawLogo !== 'undefined'
      ? normalizedRawLogo
      : null);
  const logoUrl = logo;
