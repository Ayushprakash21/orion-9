...
export const AdminOverview = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* Primary governed control plane entry */}
      <NavLink
        to="/admin/control-center"
        className="group flex flex-col gap-4 rounded-lg border border-cyan-500/30 bg-cyan-500/[0.035] p-5 transition-colors hover:border-cyan-400/50 hover:bg-cyan-500/[0.06]"
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="text-[10px] font-mono uppercase tracking-[0.2em] text-cyan-400">Admin Control Plane</div>
            <h2 className="mt-1 text-base font-semibold text-os-text-primary">AI + Manual Control Center</h2>
            <p className="mt-1 max-w-3xl text-xs leading-relaxed text-os-text-secondary">
              Manage every domain through Domain → Capability → Policy → Manual / AI Copilot / AI Autopilot → Governance → Audit.
            </p>
          </div>
          <div className="inline-flex shrink-0 items-center gap-2 rounded-md border border-cyan-500/30 bg-cyan-500/10 px-3 py-2 text-xs font-medium text-cyan-300 group-hover:bg-cyan-500/15">
            Open Control Center <ArrowRight size={13} />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-6">
          {['Domain','Capability','Policy','Operating Mode','Governance','Audit'].map(step => (
            <div key={step} className="rounded border border-os-border bg-black/20 px-2 py-2 text-center text-[10px] font-mono uppercase tracking-wide text-os-text-muted">{step}</div>
          ))}
        </div>
      </NavLink>
