  ...
  return (
    <div className="orion-admin-shell flex flex-col h-screen w-full bg-os-bg text-os-text-secondary font-sans overflow-hidden box-border">
      ...
      <header className="orion-global-topbar flex items-center justify-between h-[48px] min-h-[48px] max-h-[48px] px-3 md:px-4 bg-[#080a0d] border-b border-os-border shrink-0 box-border w-full z-[10000] relative">
        <div className="flex items-center gap-4 sm:gap-6 shrink-0 justify-start">
          <button 
            ...
            className="p-1.5 -ml-1 text-os-text-secondary hover:text-os-text-primary transition-colors lg:hidden rounded"
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 w-full relative">
        ...
        {sidebarOpen && (
          <div 
            className="absolute inset-0 z-40 bg-black/80 lg:hidden backdrop-blur-sm"
        ...
        <aside 
          className={cn(
            "absolute inset-y-0 left-0 z-50 w-[min(280px,calc(100vw-24px))] bg-os-bg border-r border-os-border text-os-text-secondary transition-transform duration-300 ease-in-out lg:relative flex flex-col shrink-0 h-full box-border",
            sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
          )}
        >
          {/* Mobile close button for sidebar */}
          <div className="flex h-14 lg:hidden w-full items-center justify-end px-4 border-b border-os-border shrink-0 bg-os-bg">
            <button 
              type="button"
              onClick={() => setSidebarOpen(false)}
          ...
          <nav className="flex-1 overflow-y-auto p-4 pt-4 lg:pt-6 space-y-1">
