...
export function OrionLiveWallpaper() {
  ...
  useEffect(() => {
    ...
    const getGlobe = () => {
      ...
      const radius = Math.min(width * 0.175, height * 0.30);
      return {
        x: width * 0.535 + parallaxX * 0.45,
        y: height * 0.505 + parallaxY * 0.35,
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-0" aria-hidden="true">
      ...
      {/* Reference-locked brand composition. The logo and statement occupy a fixed left visual lane;
          only the world model behind them moves. */}
      <div className="orion-wallpaper-brand absolute left-[clamp(48px,4.75vw,80px)] top-[clamp(14px,4.8vh,48px)] w-[clamp(220px,15vw,270px)]">
        {logoSrc ? (
          <img src={logoSrc} alt="ORION-9" className="orion-wallpaper-logo block w-full h-auto object-contain" />
        ) : (
          <BrandLogo sizePreset="xl" variant="full" className="orion-wallpaper-logo" />
        )}
      </div>

      <div className="orion-wallpaper-tagline absolute left-[clamp(48px,4.75vw,80px)] top-[clamp(210px,29.5vh,282px)] w-[clamp(300px,22vw,380px)]">
        <div className="text-[11px] sm:text-xs md:text-[13px] font-mono font-medium tracking-[0.24em] leading-[1.65] text-white/80 uppercase">
          CONNECTED INTELLIGENCE<br />FOR A MORE RESILIENT<br />TOMORROW
        </div>
        <div className="mt-5 h-[2px] w-12 bg-gradient-to-r from-cyan-300 via-cyan-400 to-blue-500 opacity-90" />
      </div>
