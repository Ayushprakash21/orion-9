...
export const SupplyChainPulse: React.FC = () => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-os-surface border border-os-border p-3 rounded-lg shadow-xl min-w-[150px]">
          <div className="text-[10px] text-os-text-muted uppercase tracking-wider mb-2 pb-2 border-b border-os-border">
            {label}
          </div>
          <div className="space-y-1.5">
            {payload.map((entry: any, index: number) => (
              <div key={index} className="flex justify-between items-center text-xs">
                ...
                <span className="font-mono text-os-text-primary ml-4">{entry.value}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="bg-os-surface border border-os-border rounded-xl flex flex-col w-full h-[450px]">
      
      {/* HEADER */}
      <div className="p-4 sm:p-5 border-b border-os-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="text-[11px] uppercase tracking-wider text-os-text-primary font-semibold flex items-center gap-2">
            <Activity size={14} className="text-[#30D158]" />
            Supply Chain Pulse
          </div>
          <div className="text-[10px] text-os-text-muted uppercase tracking-wider mt-1">Live operational activity across inventory, orders and logistics.</div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <select 
            ...
            className="bg-os-surface border border-os-border text-xs text-os-text-secondary rounded-md px-2 py-1 outline-none"
          >
      ...
      <div className="flex-1 p-4 relative min-h-0 overflow-hidden">
        {hasData ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-center">
            <Clock size={32} className="text-os-text-muted mb-3" />
            <div className="text-sm text-os-text-primary font-medium mb-1">No activity history available yet</div>
            <div className="text-xs text-os-text-muted max-w-xs">Connect a data source or perform an import to populate this view with operational events.</div>
          </div>
        )}

        {/* ACTIVITY DETAIL DRAWER OVERLAY */}
        {selectedBucket && (
          <div className="absolute top-0 right-0 w-full sm:w-80 h-full bg-os-surface border-l border-os-border shadow-2xl flex flex-col z-20 transition-transform transform translate-x-0 overflow-hidden">
            <div className="p-4 border-b border-os-border flex justify-between items-center bg-os-surface">
              <div className="text-[11px] uppercase tracking-wider text-os-text-primary font-semibold flex items-center gap-2">
                <Clock size={14} className="text-os-text-secondary" />
                Activity Detail
              </div>
              <button onClick={() => setSelectedBucket(null)} className="text-os-text-muted hover:text-os-text-primary transition-colors">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-5">
              <div className="text-center pb-4 border-b border-os-border">
                <div className="text-[10px] text-os-text-muted uppercase tracking-wider mb-1">Time Period</div>
                <div className="text-sm font-mono text-os-text-primary">{selectedBucket.timeStr}</div>
              </div>

              {selectedBucket.inventoryEvents.length > 0 && (
                <div>
                  <div className="text-[10px] text-os-text-muted uppercase tracking-wider mb-2 flex justify-between">
                    <span>Inventory Updates</span>
                    <span className="text-os-text-primary">{selectedBucket.inventoryEvents.length}</span>
                  </div>
                  <div className="space-y-2">
                    {selectedBucket.inventoryEvents.slice(0, 5).map((inv: any, i: number) => (
                      <div key={i} onClick={() => openEntity('inventory', inv.id)} className="p-2 bg-os-surface border border-os-border hover:border-os-border rounded-md text-xs cursor-pointer flex justify-between items-center transition-colors">
                        <span className="font-mono text-os-text-secondary">{inv.id}</span>
                        <ArrowRight size={12} className="text-os-text-muted" />
                      </div>
                    ))}
                    {selectedBucket.inventoryEvents.length > 5 && <div className="text-[10px] text-os-text-muted italic text-center">+ {selectedBucket.inventoryEvents.length - 5} more</div>}
                  </div>
                </div>
              )}
              ...
              {selectedBucket.poEvents.length > 0 && (
                <div>
                  <div className="text-[10px] text-os-text-muted uppercase tracking-wider mb-2 flex justify-between">
                    ...
                    <span className="text-os-text-primary">{selectedBucket.poEvents.length}</span>
                  </div>
                  <div className="space-y-2">
                    {selectedBucket.poEvents.slice(0, 5).map((po: any, i: number) => (
                      <div key={i} onClick={() => openEntity('po', po.id)} className="p-2 bg-os-surface border border-os-border hover:border-os-border rounded-md text-xs cursor-pointer flex justify-between items-center transition-colors">
                        <span className="font-mono text-os-text-secondary">{po.id}</span>
                        <ArrowRight size={12} className="text-os-text-muted" />
                      </div>
                    ))}
                    {selectedBucket.poEvents.length > 5 && <div className="text-[10px] text-os-text-muted italic text-center">+ {selectedBucket.poEvents.length - 5} more</div>}
                  </div>
                </div>
              )}
              ...
              {selectedBucket.shipmentEvents.length > 0 && (
                <div>
                  <div className="text-[10px] text-os-text-muted uppercase tracking-wider mb-2 flex justify-between">
                    <span>Shipments</span>
                    <span className="text-os-text-primary">{selectedBucket.shipmentEvents.length}</span>
                  </div>
                  <div className="space-y-2">
                    {selectedBucket.shipmentEvents.slice(0, 5).map((ship: any, i: number) => (
                      <div key={i} onClick={() => openEntity('shipment', ship.id)} className="p-2 bg-os-surface border border-os-border hover:border-os-border rounded-md text-xs cursor-pointer flex justify-between items-center transition-colors">
                        <span className="font-mono text-os-text-secondary">{ship.id}</span>
                        <ArrowRight size={12} className="text-os-text-muted" />
                      </div>
                    ))}
                    {selectedBucket.shipmentEvents.length > 5 && <div className="text-[10px] text-os-text-muted italic text-center">+ {selectedBucket.shipmentEvents.length - 5} more</div>}
                  </div>
                </div>
              )}
              ...
              {selectedBucket.exceptionEvents.length > 0 && (
                <div>
                  <div className="text-[10px] text-os-text-muted uppercase tracking-wider mb-2 flex justify-between">
                    ...
                    <span className="text-os-text-primary">{selectedBucket.exceptionEvents.length}</span>
                  </div>
                  <div className="space-y-2">
                    {selectedBucket.exceptionEvents.slice(0, 5).map((ex: any, i: number) => (
                      <div key={i} onClick={() => openEntity('exception', ex.id)} className="p-2 bg-os-surface border border-os-border hover:border-os-border rounded-md text-xs cursor-pointer flex justify-between items-center transition-colors">
                        <span className="truncate pr-2 text-os-text-secondary">{ex.type}</span>
                        <ArrowRight size={12} className="text-os-text-muted flex-shrink-0" />
                      </div>
                    ))}
                    {selectedBucket.exceptionEvents.length > 5 && <div className="text-[10px] text-os-text-muted italic text-center">+ {selectedBucket.exceptionEvents.length - 5} more</div>}
                  </div>
                </div>
              )}
            </div>
      ...
      {/* FOOTER INSIGHT */}
      <div className="p-3 bg-os-surface border-t border-os-border rounded-b-xl flex items-center justify-between">
        <div className="text-xs text-os-text-secondary flex items-center gap-2 px-2">
          <BrainCircuit size={14} className="text-os-text-muted" />
          {getInsight()}
        </div>
        <button className="text-[10px] uppercase font-mono px-3 py-1.5 bg-os-surface hover:bg-os-surface-hover text-os-text-primary border border-os-border rounded-md transition-colors flex items-center gap-1.5">
          Ask ORION
        </button>
      </div>
