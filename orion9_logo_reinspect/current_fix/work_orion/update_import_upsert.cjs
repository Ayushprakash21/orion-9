...
const importDataFunction = `  const importData = async (entityType: string, newRecords: any[], filename: string) => {
...
const startIndex = code.indexOf('const importData = async');
const endIndex = code.indexOf('const updateSettings = async', startIndex);

code = code.substring(0, startIndex) + importDataFunction + '\n\n  ' + code.substring(endIndex);
