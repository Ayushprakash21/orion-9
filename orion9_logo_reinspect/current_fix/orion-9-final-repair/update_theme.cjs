              ...
              >
                ...
                <div 
                  className={\`absolute w-[18px] h-[18px] rounded-full shadow-[0_1px_3px_rgba(0,0,0,0.3)] transform transition-transform duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] flex items-center justify-center \${
                    settings?.theme === 'light' ? 'translate-x-[25px] bg-white' : 'translate-x-[3px] bg-[#292C2F]'
                  }\`}
                />
              </div>
            </button>`;

code = code.replace(/<button\s+type="button"\s+onClick=\{\(\) => updateSettings\(\{ theme: settings\?\.theme === 'light' \? 'dark' : 'light' \}\)\}\s+className="w-full flex items-center justify-between px-3 py-2 text-xs text-os-text-secondary hover:text-os-text-primary hover:bg-os-surface-hover rounded-md transition-colors text-left"[\s\S]*?<\/button>/, newButton);
