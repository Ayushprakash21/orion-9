...
export const AdminBranding = () => {
  const [appName] = useState('ORION-9');
  ...
  useEffect(() => {
    const loadBranding = async () => {
      try {
        ...
        // Product identity is fixed: ORION-9. Only descriptor/logo are configurable here.
  ...
  const handleReset = async () => {
    if (window.confirm("Are you sure you want to reset branding to defaults?")) {
      ...
      // Product identity is fixed: ORION-9.
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <input 
                ...
                value="ORION-9"
                readOnly
                className="w-full bg-os-surface border border-os-border rounded p-2.5 text-sm text-os-text-secondary focus:outline-none cursor-not-allowed opacity-90"
                aria-readonly="true"
              />
              <p className="text-[10px] text-os-text-muted mt-1">ORION-9 is the fixed platform identity. The descriptor and logo can be managed below.</p>
            </div>
            <div className="space-y-1">
