...
import { formatCurrency, formatCurrencyPair, formatNumber } from '../lib/formatters';
...
export const Suppliers = () => {
  ...
  const { openSupplierContextMenu } = useEntityContextMenu();
    ...
    return enrichedSuppliers.filter(item => {
      ...
      if (searchQuery) {
        ...
        return (
          ...
          Boolean(item.name?.toLowerCase().includes(q)) ||
  ...
  const atRiskSpendFormatted = formatCurrencyPair(atRiskSpend, currency);
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 xl:px-10 py-6 w-full max-w-[1680px] mx-auto space-y-6 box-border min-w-0">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <KPICard 
          label="At-Risk Spend" 
          value={atRiskSpendFormatted.compact} 
          exactValue={atRiskSpendFormatted.exact}
          subLabel="Watchlist/Risk"
          status={atRiskSpend > 0 ? "critical" : "neutral"} 
        />
      </div>
        ...
        <div className="sm:hidden divide-y divide-os-border w-full">
          {filteredSuppliers.map((item) => (
            <MobileRecordCard
              ...
              onContextMenu={(e) => openSupplierContextMenu(e, item)}
              title={item.id}
              subtitle={item.name}
              statusNode={
                <StatusBadge 
              ...
              {filteredSuppliers.map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right whitespace-nowrap font-mono">
                    {(() => {
                      const sp = formatCurrencyPair(item.spend, currency);
                      return (
                        <div>
                          <div className="text-os-text-primary font-semibold" title={`Exact: ${sp.exact}`}>
                            {sp.compact}
                          </div>
                          <div className="text-[9px] text-os-text-muted select-all truncate max-w-[110px] ml-auto">
                            {sp.exact}
                          </div>
                        </div>
                      );
