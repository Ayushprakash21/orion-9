...
export class DecisionEngine {
  ...
  static generateDecisionFromException(
    ...
    options.push(this.createOption('OPT-1', 'DO NOTHING', 'Accept current situation and monitor.', 0, 'None', 'Baseline risk', exception, impact));

    if (exception.type === 'Stock-Out Risk' || exception.type === 'Shipment Delay' || exception.type === 'PO Overdue') {
      if (impactedPo) {
        options.push(this.createOption('OPT-2', 'EXPEDITE PO', `Expedite delivery for ${impactedPo.id}`, impactedPo.totalValue * 0.1, 'Prevent stockout', 'Additional logistics cost', exception, impact, { type: 'Expedite PO', params: { poId: impactedPo.id, days: 5 } }));
      }
      if (impactedInv) {
        options.push(this.createOption('OPT-3', 'TRANSFER INVENTORY', `Transfer stock from another warehouse`, 500, 'Immediate availability', 'Logistics cost', exception, impact));
      }
    }
  ...
  private static createOption(
    ...
    exception: Exception, impact: DecisionImpact,
    simAction?: any
  ): DecisionOption {
    const baseExposure = Math.round(impact?.estimatedRevenueExposure || 0);
    const baseStockouts = (impact?.quantityAtRisk && impact.quantityAtRisk > 0) ? 1 : 0;
    
    let simResult = {
       before: { financialExposure: baseExposure, projectedStockouts: baseStockouts },
       after: { financialExposure: baseExposure, projectedStockouts: baseStockouts },
       delta: { exposureDelta: 0, stockoutsDelta: 0 }
    };

    if (name === 'EXPEDITE PO') {
       const reducedExposure = Math.round(baseExposure * 0.15);
       simResult.after = { financialExposure: reducedExposure, projectedStockouts: 0 };
       simResult.delta = { exposureDelta: reducedExposure - baseExposure, stockoutsDelta: -baseStockouts };
    } else if (name === 'TRANSFER INVENTORY') {
       const reducedExposure = Math.round(baseExposure * 0.1);
       simResult.after = { financialExposure: reducedExposure, projectedStockouts: 0 };
       simResult.delta = { exposureDelta: reducedExposure - baseExposure, stockoutsDelta: -baseStockouts };
    }
