...
export async function generatePlatformIntelligencePdf(data: PlatformReportData): Promise<void> {
  ...
  doc.text(
    'Strategic cognitive synthesis combining Gemini 2.5 Flash reasoning with Orion deterministic verification engines.',
    margin,
    currentY
  );
  ...
  const metaItems = [
    ...
    { label: 'AI REASONING ENGINE', value: `Gemini 2.5 Flash (${data.analysis.confidenceScore}% Grounded)`, x: margin + colW * 2 + 4, y: metaY2 },
  ];
