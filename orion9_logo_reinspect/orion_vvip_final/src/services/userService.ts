  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      ...
      if (userIndex === -1) {
        ...
        // Repair only the known legacy seed label left by older ORION SCM builds.
        if (['Orion SCM OS User', 'ORION SCM OS User'].includes(parsed[userIndex].fullName)) {
          parsed[userIndex].fullName = 'Orion-9 User';
          needsSave = true;
        }
      }
