...
export function normalizeBranding(raw: any): BrandingConfig {
  ...
  // ORION-9 is the canonical product identity. Older persisted branding values
  // are normalized here so legacy localStorage/IndexedDB/server values cannot
  // resurrect the retired "ORION-9" name in the active UI.
  const canonicalName = (value: any, fallback: string): string => {
    const normalized = safeStr(value, fallback);
    const legacy = normalized.replace(/\s+/g, ' ').trim().toLowerCase();
    if (legacy === 'orion scm os' || legacy === 'orion 9 os' || legacy === 'orion 9 scm os' || legacy === 'orion scm') {
      return 'ORION-9';
    }
    return normalized;
  };

  const osName = canonicalName(raw.osName || raw.appName || raw.productName || raw.applicationName, defaultBranding.osName);
  const productName = canonicalName(raw.productName || raw.applicationName || raw.appName || raw.osName, defaultBranding.productName);
  const appName = canonicalName(raw.appName || productName || osName, defaultBranding.appName);
  const applicationName = canonicalName(raw.applicationName || appName || productName, defaultBranding.applicationName);
