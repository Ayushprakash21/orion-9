...
export const OrionPowerOnScreen: React.FC<OrionPowerOnScreenProps> = ({ isInitializing, onPowerOn, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[10000] bg-[#020305] flex flex-col items-center justify-between overflow-hidden font-sans select-none py-[8vh]">
      
      {/* Cinematic Vignette */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_20%,#000000_100%)] pointer-events-none z-10" />

      {/* ZONE 1: TOP BRANDING (ALWAYS VISIBLE AND ABOVE CORE) */}
      <div className="relative z-30 flex flex-col items-center shrink-0">
        <BrandLogo size={48} variant="full-descriptor" className="items-center flex-col gap-3 scale-110 drop-shadow-[0_0_15px_rgba(0,242,254,0.15)]" />
      </div>

      {/* ZONE 2: CENTER INTELLIGENCE CORE */}
      <div className="relative flex-1 w-full flex items-center justify-center my-8 z-20 pointer-events-none">
        
        {/* SVG Network Canvas */}
        <div className="absolute inset-0 flex items-center justify-center">
          <svg className="w-[800px] h-[800px] max-w-none opacity-80" viewBox="-400 -400 800 800">
            <defs>
        ...
        <button
          ...
          className={`
            relative w-28 h-28 sm:w-36 sm:h-36 rounded-full flex items-center justify-center pointer-events-auto
      ...
      {/* ZONE 3: BOTTOM STATUS */}
      <div className={`relative z-30 shrink-0 flex flex-col items-center transition-opacity duration-1000
        ${timeMs > 8500 ? 'opacity-0' : 'opacity-100'}`}>
        <div className="flex items-center gap-4">
          <div className="h-px w-16 bg-gradient-to-r from-transparent to-[#00F2FE]/30" />
          <p className={`
            text-[10px] sm:text-xs font-mono tracking-[0.3em] sm:tracking-[0.4em] uppercase transition-colors duration-500
