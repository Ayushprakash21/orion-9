...
const PHONE_COUNTRIES = [
  ...
  ['GB', 'United Kingdom', '+44'], ['AU', 'Australia', '+61'], ['AE', 'UAE', '+971'],
  ['SG', 'Singapore', '+65'], ['MY', 'Malaysia', '+60'], ['NZ', 'New Zealand', '+64'],
  ['DE', 'Germany', '+49'], ['FR', 'France', '+33'], ['IT', 'Italy', '+39'],
  ['ES', 'Spain', '+34'], ['NL', 'Netherlands', '+31'], ['CH', 'Switzerland', '+41'],
  ['SE', 'Sweden', '+46'], ['NO', 'Norway', '+47'], ['DK', 'Denmark', '+45'],
  ['JP', 'Japan', '+81'], ['KR', 'South Korea', '+82'], ['CN', 'China', '+86'],
  ['HK', 'Hong Kong', '+852'], ['TW', 'Taiwan', '+886'], ['TH', 'Thailand', '+66'],
  ['ID', 'Indonesia', '+62'], ['PH', 'Philippines', '+63'], ['VN', 'Vietnam', '+84'],
  ['BR', 'Brazil', '+55'], ['MX', 'Mexico', '+52'], ['AR', 'Argentina', '+54'],
  ['ZA', 'South Africa', '+27'], ['NG', 'Nigeria', '+234'], ['KE', 'Kenya', '+254'],
  ['EG', 'Egypt', '+20'], ['TR', 'Türkiye', '+90'], ['SA', 'Saudi Arabia', '+966'],
  ['IL', 'Israel', '+972'], ['RU', 'Russia', '+7'], ['UA', 'Ukraine', '+380'],
  ['PL', 'Poland', '+48'], ['PT', 'Portugal', '+351'], ['IE', 'Ireland', '+353'],
  ['BD', 'Bangladesh', '+880'], ['PK', 'Pakistan', '+92'], ['LK', 'Sri Lanka', '+94'],
  ['NP', 'Nepal', '+977']
] as const;
...
const splitPhone = (value: string | null | undefined) => {
  ...
  if (!match) return { country: '+91', number: raw };
  const known = PHONE_COUNTRIES.find(([, , code]) => code === match[1]);
  return { country: known ? match[1] : '+91', number: known ? match[2] : raw };
};
...
export const Profile = ({ initialTab = 'profile' }: { initialTab?: 'profile' | 'organization' | 'preferences' | 'security' }) => {
  ...
  useEffect(() => {
    if (!profile) return;
    const parsedPhone = splitPhone(profile.phone);
  ...
  const saveProfile = async () => {
    ...
    try {
      await userRepository.updateProfile(profile.id, {
        ...
        jobTitle,
        department,
        phone: phone.trim() ? `${phoneCountry} ${phone.trim()}` : '',
        avatarUrl
      });
  ...
  return (
    <div className="flex flex-col h-full bg-os-bg text-os-text-primary">
      <div className="shrink-0 px-4 sm:px-6 lg:px-8 py-6 border-b border-os-border box-border w-full max-w-[1120px] mx-auto text-center">
        <h1 className="text-2xl font-light tracking-tight mb-6">Account & Settings</h1>
        
        <div className="flex justify-center gap-6 border-b border-os-border/40">
          <button 
            id="tab-profile"
      ...
      <div className="flex-1 overflow-auto px-4 sm:px-6 lg:px-8 py-8 box-border w-full max-w-[1120px] mx-auto">
            ...
            <div className="animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-os-bg border border-os-border rounded-xl p-6">
                ...
                <div className="space-y-1">
                  ...
                  {isEditingProfile ? (
                    <div className="flex w-full bg-os-surface border border-os-border rounded overflow-hidden focus-within:border-blue-500/70">
                      <div className="relative shrink-0 border-r border-os-border">
                        <select
                          aria-label="Country calling code"
                          value={phoneCountry}
                          onChange={e => setPhoneCountry(e.target.value)}
                          className="h-full appearance-none bg-transparent pl-3 pr-8 py-2.5 text-sm text-os-text-primary focus:outline-none cursor-pointer"
                        >
                          {PHONE_COUNTRIES.map(([iso, country, code]) => (
                            <option key={`${iso}-${code}`} value={code}>
                              {iso} {code} · {country}
                            </option>
                          ))}
                        </select>
                        <span className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-os-text-muted text-xs">⌄</span>
                      </div>
                      <input
                        type="tel"
                        value={phone}
                        ...
                        className="min-w-0 flex-1 bg-transparent px-3 py-2.5 text-sm text-os-text-primary focus:outline-none"
                        inputMode="tel"
                      />
                    </div>
                  ) : (
