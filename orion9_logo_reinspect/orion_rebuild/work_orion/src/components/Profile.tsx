...
export const Profile = ({ initialTab = 'profile' }: { initialTab?: 'profile' | 'organization' | 'preferences' | 'security' }) => {
  ...
  // Keep the form synchronized if the authenticated profile changes elsewhere.
  useEffect(() => {
  ...
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Resize/compress before persisting so local storage is not filled by a
    // camera-resolution image. The profile picture is capped at 512px.
    ...
    reader.onload = () => {
      ...
      img.onerror = () => showToast('Unable to read that image.', 'error');
      img.src = reader.result as string;
    };
    reader.onerror = () => showToast('Unable to read that image.', 'error');
  ...
  const saveProfile = async () => {
    ...
    const cleanDisplayName = displayName.trim() || cleanName.split(' ')[0] || '';
    const cleanEmail = email.trim().toLowerCase();
    const cleanOrganization = profileOrganization.trim();

    if (!cleanName) {
      showToast('Full name is required.', 'error');
      return;
    }
    // Email is a one-time identity field: it can be entered when empty, but
    // once an email has been saved it is never overwritten from this screen.
    if (!profile.email && !cleanEmail) {
      showToast('Email address is required before the profile can be saved.', 'error');
      return;
    }
    ...
    try {
      ...
      if (!profile.email) updates.email = cleanEmail;
              ...
              <div className="flex items-center gap-6 mb-8 p-6 bg-os-bg border border-os-border rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-xs text-os-text-muted mt-1">
                    Assigned Workspace: <span className="text-blue-400 font-medium">{organization?.name || profile.organizationName || 'None'}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-os-bg border border-os-border rounded-xl p-6">
                ...
                <div className="space-y-1">
                  ...
                  {isEditingProfile && !profile.email ? (
                    ...
                    <div className="p-2.5 text-sm bg-os-surface border border-os-border rounded text-os-text-secondary flex items-center justify-between gap-2">
                      <span className="flex items-center gap-2 min-w-0"><Mail size={14} className="text-os-text-muted shrink-0" /> <span className="truncate">{profile.email || '-'}</span></span>
                      {profile.email && <span title="Email is locked after first save"><Lock size={13} className="text-os-text-muted shrink-0" /></span>}
                    </div>
                  )}
                  <p className="text-[10px] text-os-text-muted">{profile.email ? 'Locked after first save.' : 'You can set this once. It cannot be changed later.'}</p>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  {isEditingProfile ? (
                    ...
                    <div className="p-2.5 text-sm bg-os-surface border border-os-border rounded text-os-text-primary">{profile.organizationName || organization?.name || '-'}</div>
                  )}
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase tracking-wider text-os-text-muted font-semibold">Assigned Workspace</label>
            ...
            <div className="animate-in fade-in duration-300 space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-os-text-secondary">Your profile organization name is editable from Profile. Workspace assignment remains administrator-controlled.</p>
                </div>
