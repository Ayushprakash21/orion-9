...
export const Profile = ({ initialTab = 'profile' }: { initialTab?: 'profile' | 'organization' | 'preferences' | 'security' }) => {
  ...
  const emailWasAlreadySaved = Boolean(profile?.email?.trim());
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      // Email is intentionally write-once for users. If an email already
      // exists, never send an email update from this screen.
      if (!emailWasAlreadySaved && email.trim()) {
  ...
  return (
    <div className="flex flex-col h-full bg-os-bg text-os-text-primary">
      <div className="shrink-0 px-4 sm:px-6 lg:px-8 py-6 border-b border-os-border box-border w-full max-w-[1100px] mx-auto">
      ...
      <div className="flex-1 overflow-auto px-4 sm:px-6 lg:px-8 py-8 box-border w-full max-w-[1100px] mx-auto">
            ...
            <div className="animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-os-bg border border-os-border rounded-xl p-6">
                ...
                <div className="space-y-1">
                  ...
                  {!emailWasAlreadySaved && isEditingProfile ? (
                    <input
                      type="email"
                      value={email}
                    ...
                    <div className="p-2.5 text-sm bg-os-surface border border-os-border rounded text-os-text-secondary flex items-center gap-2">
                      ...
                      {emailWasAlreadySaved && <span className="ml-auto text-[9px] uppercase tracking-wider text-emerald-400">Locked</span>}
                    </div>
                  )}
                </div>
                  ...
                  {isEditingProfile ? (
                    <div className="flex gap-2">
                      <select
                        ...
                        className="w-[150px] shrink-0 bg-os-surface border border-os-border rounded p-2.5 text-sm text-os-text-primary focus:outline-none focus:border-[#00F2FE] cursor-pointer"
                      >
                        {PHONE_COUNTRIES.map(([code, country]) => (
