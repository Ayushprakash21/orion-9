...
export class BaseConnector {
  ...
  async testConnection(): Promise<{ success: boolean; latency: string; message: string }> {
    ...
    if (!this.config.apiKey && !this.config.username && this.type !== 'FILE') {
      ...
      return { success: false, latency: '-', message: 'Authentication failed: Invalid credentials or token unauthorized.' };
    }
    
    // Perform simulated real test
    this.connectionStatus = 'CONNECTED';
    this.latency = 'SIMULATED';
    ...
    return { success: true, latency: 'SIMULATED', message: 'Connection established successfully.' };
  }
