...
export const Suppliers = () => {
  ...
  const enrichedSuppliers = useMemo(() => {
    return suppliers.map(sup => {
      const { score, status, riskLevel } = SupplierEngine.calculateScore(sup, settings);
          ...
          {filteredSuppliers.map((item) => (
            <MobileRecordCard
              ...
              fields={[
                ...
                { label: 'Risk', value: item.riskLevel, valueClassName: `font-mono ${item.riskLevel === 'High' ? 'text-[#FF453A]' : item.riskLevel === 'Medium' ? 'text-[#FF9F0A]' : 'text-[#B3B3B3]'}` }
              ]}
            />
          ))}
