...
export const Integrations = () => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      setTestResult({ success: false, latency: '-', message: err?.message || 'Connection test failed.' });
    } finally {
      setIsTesting(false);
    }
  };
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 xl:px-10 py-6 w-full max-w-[1680px] mx-auto space-y-8 box-border min-w-0">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-os-surface border border-os-border rounded-sm text-xs text-os-text-secondary uppercase tracking-widest hover:bg-os-surface-hover transition-colors"
          >
            <RefreshCw size={14} />
            Sync All Connected
          </button>
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-os-surface-elevated border border-os-border text-os-text-primary rounded-sm text-xs uppercase tracking-widest hover:bg-os-surface-hover transition-colors"
          >
            <Network size={14} />
            Add Connection
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-os-bg/50 border border-os-border p-4 rounded-sm flex items-center gap-4">
          <div className="w-10 h-10 rounded-sm bg-os-surface-hover border border-os-border flex items-center justify-center text-os-text-primary">
            <Globe size={18} />
          </div>
          <div>
      ...
      {modalMode === 'wizard' && selectedConnector && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-os-bg border border-os-border w-full max-w-3xl rounded-sm p-6 space-y-6 relative max-h-[90vh] overflow-y-auto">
            <button 
            ...
            <div>
              <div className="text-[10px] uppercase font-mono tracking-widest text-os-text-primary">INTEGRATION WIZARD</div>
              <h3 className="text-xl font-light text-os-text-primary">Connect {selectedConnector.name}</h3>
              <p className="text-xs text-os-text-secondary mt-1">ERP Integration</p>
            </div>

            {/* Wizard Steps Bar */}
            <div className="flex flex-wrap sm:grid sm:grid-cols-5 gap-2 text-[10px] font-mono border-b border-os-border pb-4">
              <div className={wizardStep === 1 ? 'text-os-text-primary font-bold' : 'text-os-text-muted'}><span className="sm:hidden">1.</span><span className="hidden sm:inline">1. PURPOSE</span></div>
              <div className={wizardStep === 2 ? 'text-os-text-primary font-bold' : 'text-os-text-muted'}><span className="sm:hidden">2.</span><span className="hidden sm:inline">2. SYSTEM</span></div>
              <div className={wizardStep === 3 ? 'text-os-text-primary font-bold' : 'text-os-text-muted'}><span className="sm:hidden">3.</span><span className="hidden sm:inline">3. AUTH & TEST</span></div>
              <div className={wizardStep === 4 ? 'text-os-text-primary font-bold' : 'text-os-text-muted'}><span className="sm:hidden">4.</span><span className="hidden sm:inline">4. DISCOVER & MAP</span></div>
            ...
            {wizardStep === 1 && (
              <div className="space-y-4">
                <div>
                  <label className="block text-xs uppercase tracking-wider text-os-text-secondary mb-2 font-medium">What do you want ORION to monitor?</label>
                  <p className="text-xs text-os-text-muted mb-4">Select the operational domains you intend to oversee through this connection.</p>
                    ...
                    {['Inventory', 'Procurement', 'Supplier Performance', 'Inbound', 'Outbound', 'Logistics', 'Demand'].map((purpose) => {
                      ...
                      return (
                        <div 
                          ...
                          className={`p-3 rounded-sm border cursor-pointer text-xs font-mono flex items-center justify-between transition-colors ${selected ? 'bg-os-surface-elevated border-os-border/40 text-cyan-300' : 'bg-os-surface-secondary border-os-border text-os-text-secondary hover:border-os-border-strong'}`}
                        >
                          <span>{purpose}</span>
                          <span className={`w-4 h-4 rounded-sm border flex items-center justify-center text-[10px] ${selected ? 'bg-cyan-500 border-os-border text-black font-bold' : 'border-os-border-strong'}`}>
                            {selected ? '✓' : ''}
                          </span>
                        </div>
                      );
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-os-surface-secondary border border-os-border rounded-sm p-2.5 text-xs font-mono text-os-text-primary"
                  />
                </div>
                <div>
                  <label className="block text-xs uppercase tracking-wider text-os-text-secondary mb-1">Environment</label>
                  <select 
                  ...
                  {configForm.environment === 'Production' && (
                    <div className="mt-3 p-3 bg-amber-500/10 border border-amber-500/30 rounded-sm text-xs font-mono text-amber-300 space-y-2">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer pt-1">
                        ...
                        />
                        <span className="text-os-text-primary">I understand this will create a production integration.</span>
                      </label>
                    </div>
                  )}
                </div>
                <div>
                  <label className="block text-xs uppercase tracking-wider text-os-text-secondary mb-1">Base URL / API Endpoint</label>
                  <input 
                    type="text" 
                ...
                <div className="flex justify-between pt-4">
                  <button 
                    ...
                    className="px-4 py-2 bg-white/5 border border-os-border text-os-text-secondary text-xs uppercase tracking-widest rounded-sm"
                  >
                    Back
                  </button>
                  <button 
            ...
            {wizardStep === 3 && (
              <div className="space-y-6">
                <div>
                  <label className="block text-xs uppercase tracking-wider text-os-text-secondary mb-1">Authentication Architecture</label>
                  <select 
                ...
                <div>
                  <label className="block text-xs uppercase tracking-wider text-os-text-secondary mb-1">API Key / Secret Token / Credentials</label>
                  <input 
                    type="password" 
                    ...
                    className="w-full bg-os-surface-secondary border border-os-border rounded-sm p-2.5 text-xs font-mono text-os-text-primary"
                  />
                  <p className="text-[10px] text-os-text-muted mt-1">Secrets are encrypted in secure storage and never exposed in logs or UI.</p>
                </div>

                <div className="bg-os-surface-secondary border border-os-border p-4 rounded-sm space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-xs uppercase font-mono text-os-text-secondary">Connection Test Handshake</span>
                    <button 
                      onClick={handleTestConnection}
                      disabled={isTesting}
                      className="px-4 py-1.5 bg-os-surface-hover border border-os-border/40 text-os-text-primary text-xs uppercase tracking-widest rounded-sm hover:bg-cyan-500/30 transition-colors"
                    >
                  ...
                  {testResult && (
                    <div className={`p-3 rounded-sm border text-xs font-mono ${testResult.success ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-rose-500/10 border-rose-500/30 text-rose-400'}`}>
                      <div>STATUS: {testResult.success ? 'CONNECTED (SIMULATED)' : 'FAILED'}</div>
                      <div className="mt-1 text-[11px] text-os-text-secondary">{testResult.message}</div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between pt-4">
                  ...
                  <button 
                    ...
                    className={`px-6 py-2 text-xs font-bold uppercase tracking-widest rounded-sm transition-colors ${testResult?.success ? 'bg-cyan-500 text-black hover:bg-cyan-400' : 'bg-white/10 text-os-text-muted cursor-not-allowed'}`}
                  >
                    Next: Discover & Map
                  </button>
                </div>
            ...
            {wizardStep === 4 && (
              <div className="space-y-6">
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <div className="text-xs uppercase font-mono text-os-text-primary">Data Domain Discovery</div>
                    <button 
                      onClick={() => setDiscoveredDomains(selectedConnector.getDataDomains())}
                      className="px-3 py-1 bg-white/5 border border-os-border text-os-text-secondary text-xs uppercase tracking-widest rounded-sm hover:opacity-90/10"
                    >
                      Discover Data
                    </button>
                  </div>
                ...
                <div className="flex justify-between pt-4">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="px-4 py-2 bg-white/10 border border-os-border-strong text-os-text-primary text-xs uppercase tracking-widest rounded-sm hover:opacity-90/20"
                    >
                      Run Validation
                    </button>
                    <button 
