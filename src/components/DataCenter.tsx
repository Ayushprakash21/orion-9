  ...
  const processImportData = (rawData: any[], entityType: string, filename: string) => {
    ...
    if (entityType === 'inventory') {
      parsed.valid.forEach((record, index) => {
        if (!products.find(p => p.id === record.productId)) warnings.push({ row: index + 1, message: `WARNING: Referenced entity not found (SKU: ${record.productId})`});
      ...
      parsed.valid.forEach((record, index) => {
        ...
        record.lines.forEach((line: any) => {
          if (!products.find(p => p.id === line.productId)) warnings.push({ row: record.poNumber, message: `WARNING: Referenced entity not found (SKU: ${line.productId})`});
        });
                      ...
                      <MobileRecordCard
                        ...
                        statusNode={
                          <span className={`text-[10px] uppercase tracking-widest px-2 py-0.5 rounded-sm border ${
                              hist.status === 'Success' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' :
                              hist.status === 'Failed' ? 'text-rose-400 bg-rose-500/10 border-rose-500/20' :
