  ...
  return (
    <div className="bg-[#151515] border border-[#2A2A2A] p-4 sm:p-6 rounded-xl w-full relative">
        ...
        <div className="grid grid-cols-4 gap-4 relative z-10 mb-12">
          {nodesInfo.slice(0, 4).map((node, i) => (
            <div key={node.id} className={`relative flex flex-col items-center ${activeNode === node.id ? 'z-50' : 'z-10'}`}>
              {/* Connector right */}
              {i < 3 && (
                <div className="absolute top-[40px] left-[70%] right-[-30%] h-[2px] bg-[#2A2A2A] z-0">
      ...
      <div className="md:hidden flex flex-col items-center gap-6 relative py-4">
        ...
        <div className="absolute top-8 bottom-8 left-[38px] w-[2px] bg-[#2A2A2A] z-0">
        ...
        {nodesInfo.map((node, i) => (
          <div key={node.id} className="relative z-10 w-full">
            <div 
              className="bg-[#111111] border border-[#2A2A2A] active:border-[#777777] rounded-lg p-3 cursor-pointer transition-colors flex items-center gap-4"
              onClick={() => handleNodeClick(node)}
            >
              <div className="bg-[#1A1A1A] p-2 rounded-md shrink-0">
                <node.icon size={18} className="text-[#F5F5F5]" />
              </div>
            ...
            {activeNode === node.id && (
              <div className="mt-2 w-full bg-[#1A1A1A] border border-[#333333] rounded-lg p-4 z-50">
                {node.details.map((d, idx) => (
