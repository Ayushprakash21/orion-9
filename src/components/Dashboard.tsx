    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 pb-4 border-b border-[#2A2A2A]">
        <div>
          ...
          <p className="text-[11px] text-[#777777] uppercase tracking-wider mt-1">ORION-9 AI SUPPLY CHAIN OPERATING SYSTEM</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* SUPPLY CHAIN HEALTH */}
        <div className="col-span-1 bg-[#151515] border border-[#2A2A2A] p-6 rounded-xl relative overflow-hidden group">
        ...
        {/* ORION ASSESSMENT */}
        <div className="col-span-1 lg:col-span-2 bg-[#151515] border border-[#2A2A2A] p-6 rounded-xl relative">
           <div className="text-[11px] uppercase tracking-wider text-[#F5F5F5] font-semibold mb-4 flex items-center gap-2">
             <BrainCircuit size={16} className="text-[#B3B3B3]" />
             ORION AI Assessment
           </div>
           ...
           {primaryIssue ? (
             <div className="space-y-4">
               <div>
                 ...
                 <div className="text-base text-[#FF453A] font-medium">{primaryIssue.type} on {primaryIssue.entityId}</div>
               </div>
               
               {rootCauseChain && (
                 <div className="bg-[#111111] p-3.5 rounded-lg border border-[#2A2A2A]">
                    <div className="text-[10px] text-[#777777] uppercase tracking-wider mb-2.5 flex items-center gap-2">
                      <Network size={12} className="text-[#B3B3B3]" />
                      Root Cause Chain Detected
                    </div>
                    <div className="flex items-center gap-2 overflow-x-auto pb-1">
                      {rootCauseChain.map((node, i) => (
                        <React.Fragment key={i}>
                          <div className="px-2.5 py-1 bg-[#151515] border border-[#2A2A2A] hover:border-[#777777] cursor-pointer rounded-md text-[11px] font-mono text-[#F5F5F5] whitespace-nowrap transition-colors">
                            <span className="text-[#777777] mr-1">{node.type}:</span>{node.id}
                          </div>
                          {i < rootCauseChain.length - 1 && <ArrowRight size={12} className="text-[#555555]" />}
                        </React.Fragment>
                      ))}
                    </div>
                 </div>
               ...
               <div className="grid grid-cols-2 gap-4">
                 ...
                 <div className="bg-[#111111] p-3.5 border border-[#2A2A2A] rounded-lg">
                   <div className="text-[10px] text-[#777777] uppercase tracking-wider mb-1">Business Impact</div>
            ...
            {actions.map(act => (
              <MobileRecordCard
                ...
                fields={[
                  ...
                  { label: 'Impact', value: formatCurrency(act.impact, currency) }
                ]}
              />
            ))}
          </div>
          ...
          <div className="hidden sm:block overflow-x-auto w-full">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#2A2A2A] text-xs">
                {actions.map(act => (
                  <tr key={act.id} className="hover:bg-[#202020] transition-colors">
                    <td className="p-3">
                    ...
                    <td className="p-3 font-mono text-[#F5F5F5]">{formatCurrency(act.impact, currency)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
