...
export const OrionWorldEntrySequence:React.FC<Props>=({onComplete,isAdmin=false})=>{
 ...
 useEffect(()=>{let raf=0;const tick=(t:number)=>{if(start.current===null)start.current=t;const e=t-start.current;setElapsed(e);if(e>=1500){if(!done.current){done.current=true;onComplete()}return}raf=requestAnimationFrame(tick)};raf=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf)},[onComplete]);
 ...
 return <main className={`orion-cinematic orion-world-v3 ${isAdmin?'admin':'user'}`} role="status" aria-live="polite">
   ...
   <section className={`ow3-system-map ${live?'on':''}`}><div className="ow3-map-head"><span>SESSION FABRIC</span><b>{Math.min(100,Math.floor(elapsed/1500*100))}%</b></div>{systems.map((s,i)=><div key={s} className="ow3-row" style={{animationDelay:`${i*140}ms`}}><span>{String(i+1).padStart(2,'0')}</span><b>{s}</b><em>BOUND</em></div>)}</section>
