...
export const BrandingProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const applyBranding = useCallback((config: BrandingConfig) => {
    ...
    if (typeof document !== 'undefined') {
      const name = config.appName || config.productName || config.applicationName || 'ORION-9';
...
export const useBranding = (): BrandingContextType => {
  ...
  if (!context) {
    const syncConfig = brandingRepository.getBrandingSync();
    const appName = syncConfig.appName || syncConfig.productName || syncConfig.applicationName || syncConfig.osName || 'ORION-9';
