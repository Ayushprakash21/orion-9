...
export const demoSuppliers: Supplier[] = Array.from({ length: 25 }).map((_, i) => {
  ...
  return {
    ...
    otif: isHighRisk ? 70 + Math.random() * 15 : 90 + Math.random() * 8,
    qualityRate: isHighRisk ? 85 + Math.random() * 10 : 96 + Math.random() * 4,
    leadTime: 7 + (i % 3) * 7,
    defectRate: isHighRisk ? Math.random() * 5 + 2 : Math.random() * 1.5,
    spend: 100000 + Math.random() * 900000,
  };
});
...
export const demoInventory: Inventory[] = demoProducts.map((p, i) => {
  ...
  const avgDemand = 10 + Math.floor(Math.random() * 50);
  let onHand = avgDemand * (20 + Math.random() * 20); // Healthy: 20-40 days
  if (isCritical) {
    onHand = avgDemand * (Math.random() * 5); // 0-5 days
  } else if (isExcess) {
    onHand = avgDemand * (60 + Math.random() * 40); // 60-100 days
  }
...
export const demoPurchaseOrders: PurchaseOrder[] = Array.from({ length: 150 }).map((_, i) => {
  ...
  const status = isDelayed ? 'Delayed' : statusList[Math.floor(Math.random() * statusList.length)];
  const orderDate = subDays(today, 5 + Math.floor(Math.random() * 40));
...
export const demoShipments: Shipment[] = demoPurchaseOrders.slice(0, 100).map((po, i) => {
  ...
  const delayDays = isDelayed ? 2 + Math.floor(Math.random() * 10) : 0;
  
  const shipmentStatuses: Shipment['status'][] = ['Planned', 'Picked Up', 'In Transit', 'Delivered'];
  let status: Shipment['status'] = isDelayed ? 'Delayed' : shipmentStatuses[Math.floor(Math.random() * shipmentStatuses.length)];
  ...
  return {
    ...
    status,
    delayDays,
    freightCost: 5000 + Math.random() * 20000,
  };
});
...
export const demoExceptions: Exception[] = [
  ...demoProducts.filter((_, i) => i % 15 === 0).map((p, i): Exception => ({
    id: `EXC-SKU-${i}`,
    date: subDays(today, Math.floor(Math.random() * 5)).toISOString(),
    type: 'Stock-Out Risk',
    severity: 'Critical',
    entityId: p.id,
    description: `Inventory for ${p.id} has fallen critically below safety stock levels.`,
    estimatedImpact: 25000 + Math.random() * 50000,
    status: 'Open',
    owner: 'Jane Smith',
  ...
  ...demoSuppliers.filter((_, i) => i % 5 === 0).map((s, i): Exception => ({
    id: `EXC-SUP-${i}`,
    date: subDays(today, 1 + Math.floor(Math.random() * 10)).toISOString(),
    type: 'Supplier Delay',
    severity: 'High',
    entityId: s.id,
    description: `Supplier ${s.name} OTIF has dropped below 75% over the last 30 days.`,
    estimatedImpact: 100000 + Math.random() * 200000,
  ...
  ...demoShipments.filter(s => s.delayDays > 5).map((s, i): Exception => ({
    ...
    estimatedImpact: 5000 + Math.random() * 15000,
