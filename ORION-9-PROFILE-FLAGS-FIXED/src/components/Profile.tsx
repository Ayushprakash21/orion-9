...
export const Profile = ({ initialTab = 'profile' }: { initialTab?: 'profile' | 'organization' | 'preferences' | 'security' }) => {
  ...
  const PHONE_COUNTRIES = [
    ...
    ['+61', 'Australia', '🇦🇺'], ['+64', 'New Zealand', '🇳🇿'], ['+65', 'Singapore', '🇸🇬'],
    ['+971', 'United Arab Emirates', '🇦🇪'], ['+966', 'Saudi Arabia', '🇸🇦'], ['+974', 'Qatar', '🇶🇦'],
    ['+49', 'Germany', '🇩🇪'], ['+33', 'France', '🇫🇷'], ['+39', 'Italy', '🇮🇹'], ['+34', 'Spain', '🇪🇸'],
    ['+31', 'Netherlands', '🇳🇱'], ['+81', 'Japan', '🇯🇵'], ['+82', 'South Korea', '🇰🇷'],
    ['+86', 'China', '🇨🇳'], ['+55', 'Brazil', '🇧🇷'], ['+27', 'South Africa', '🇿🇦'], ['+7', 'Russia / Kazakhstan', '🇷🇺']
  ] as const;

  const selectedPhoneCountry = PHONE_COUNTRIES.find(([code]) => code === phoneCountry) || PHONE_COUNTRIES[0];
  ...
  useEffect(() => {
    ...
    setProfileOrganization(profile.organizationName || organization?.name || '');
  }, [profile?.id, profile?.fullName, profile?.displayName, profile?.jobTitle, profile?.department, profile?.avatarUrl, profile?.organizationName, organization?.name]);

  useEffect(() => {
    const closeCountryMenu = (event: MouseEvent) => {
      const target = event.target as HTMLElement;
      if (!target.closest('[data-phone-country-picker]')) setPhoneCountryOpen(false);
    };
  ...
  useEffect(() => {
    ...
    if (match) {
      const known = PHONE_COUNTRIES.some(([code]) => code === match[1]);
      if (known) setPhoneCountry(match[1]);
      setPhoneNumber(match[2].replace(/[^0-9 ()-]/g, '').trim());
    } else {
      setPhoneCountry('+91');
                ...
                <div className="space-y-1">
                  ...
                  {isEditingProfile ? (
                    <div className="flex gap-2">
                      <div className="relative w-[190px] shrink-0" data-phone-country-picker>
                        <button
                          type="button"
                          ...
                          className="w-full h-[42px] bg-os-surface border border-os-border rounded px-3 text-sm text-os-text-primary focus:outline-none focus:border-[#00F2FE] transition-colors flex items-center gap-2.5 text-left"
                        >
                          <span className="text-[19px] leading-none" aria-hidden="true">{selectedPhoneCountry[2]}</span>
                          <span className="font-medium tabular-nums">{selectedPhoneCountry[0]}</span>
                          <span className="text-os-text-muted truncate flex-1">{selectedPhoneCountry[1]}</span>
                          <span className="text-os-text-muted text-xs">{phoneCountryOpen ? '▲' : '▼'}</span>
                        </button>
                        {phoneCountryOpen && (
                          <div
                            role="listbox"
                            aria-label="Phone country"
                            className="absolute z-[80] left-0 top-[46px] w-[300px] max-h-[320px] overflow-y-auto rounded-xl border border-white/10 bg-[#111315]/98 shadow-2xl shadow-black/50 backdrop-blur-xl p-1.5"
                          >
                            {PHONE_COUNTRIES.map(([code, country, flag]) => (
                              <button
                                key={code}
                                type="button"
                                role="option"
                                aria-selected={phoneCountry === code}
                                onClick={() => { setPhoneCountry(code); setPhoneCountryOpen(false); }}
                                className={`w-full rounded-lg px-2.5 py-2 flex items-center gap-3 text-left text-sm transition-colors ${phoneCountry === code ? 'bg-white/[0.08] text-white' : 'text-os-text-primary hover:bg-white/[0.05]'}`}
                              >
                                <span className="w-7 text-center text-[19px] leading-none" aria-hidden="true">{flag}</span>
                                <span className="w-12 shrink-0 font-medium tabular-nums">{code}</span>
                                <span className="truncate text-os-text-secondary">{country}</span>
                                {phoneCountry === code && <CheckCircle2 size={14} className="ml-auto shrink-0 text-cyan-400" />}
                              </button>
                            ))}
                          </div>
                        )}
