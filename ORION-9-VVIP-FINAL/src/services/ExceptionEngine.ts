...
export class ExceptionEngine {
  static generateExceptions(
    ...
    inventory.forEach(inv => {
      ...
      } else if (daysOfSupply !== null && daysOfSupply <= settings.lowStockDays) {
        addOrUpdate({
        date: today.toISOString(),
          type: 'Low Stock',
          severity: 'High',
    ...
    const supQualityThreshold = Number(settings?.supplierQualityThreshold) || 90;
    const delayAlertDays = Number(settings?.shipmentDelayAlertDays) ?? 2;

    suppliers.forEach(sup => {
      if (sup.otif < supHighRiskOtif) {
        addOrUpdate({
        date: today.toISOString(),
          type: 'Supplier Delay',
          severity: 'High',
          entityId: sup.id,
          description: `Supplier ${sup.name} OTIF (${sup.otif}%) is below the ${supHighRiskOtif}% threshold.`,
      ...
      if (sup.qualityRate < supQualityThreshold) {
        addOrUpdate({
        date: today.toISOString(),
    ...
    shipments.forEach(shp => {
      if (shp.delayDays >= delayAlertDays && !['Delivered', 'Cancelled'].includes(shp.status)) {
        addOrUpdate({
        date: today.toISOString(),
          type: 'Shipment Delay',
          severity: 'High',
