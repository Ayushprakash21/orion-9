  ...
  const handleDockSurfaceContextMenu = (e: React.MouseEvent) => {
    ...
    if (activeApp && activeIsRunning) {
      if (activeIsMinimized) {
        items.push({ id: 'surface-restore', label: 'Restore', icon: RotateCcw, action: () => restoreApplication(activeAppId!) });
      } else {
        items.push({ id: 'surface-minimize', label: 'Minimize', icon: Minus, action: () => minimizeApplication(activeAppId!) });
        items.push({ id: 'surface-maximize', label: 'Maximize', icon: Square, disabled: !!activeIsMaximized, action: () => maximizeApplication(activeAppId!) });
      }
      items.push({ id: 'surface-close', label: `Close ${activeApp.name}`, icon: X, danger: true, action: () => closeApplication(activeAppId!) });
      items.push({ id: 'surface-window-sep', label: '', separator: true });
    }

    items.push({ id: 'surface-launch', label: 'Open Applications...', icon: Grid, shortcut: 'F4', action: () => setLauncherOpen(true) });
    items.push({ id: 'surface-palette', label: 'Command Palette...', icon: Search, shortcut: '⌘K', action: () => setCommandPaletteOpen(true) });
    items.push({ id: 'surface-desktop-sep', label: '', separator: true });
    items.push({ id: 'surface-refresh', label: 'Refresh Desktop', icon: RefreshCw, shortcut: 'F5', action: () => {
      window.dispatchEvent(new CustomEvent('orion:desktop-refresh', { detail: { timestamp: Date.now() } }));
      showToast('Desktop telemetry refreshed', 'success', 'Desktop');
    }});
