...
export type ConnectionStatus = 'NOT CONFIGURED' | 'CONFIGURED' | 'CONFIGURATION READY' | 'AUTHENTICATED' | 'CONNECTED' | 'SYNCING' | 'WARNING' | 'FAILED' | 'DISABLED' | 'EXTERNAL CONNECTOR REQUIRED';
...
export class BaseConnector {
  ...
  async testConnection(): Promise<{ success: boolean; latency: string; message: string }> {
    ...
    if (this.type === 'FILE') {
      ...
      this.latency = `${elapsed}ms (Local)`;
      this.log('SUCCESS', 'TEST', 'Local file ingestion engine active and verified.');
      return { success: true, latency: this.latency, message: 'Local file ingestion engine active. Ready for CSV and Excel uploads.' };
    }
    ...
    if (!this.config.baseUrl || this.config.baseUrl.trim() === '' || this.config.baseUrl.includes('<customer-sap-endpoint>')) {
      ...
      return { success: false, latency: '-', message: 'Missing endpoint or invalid URL. Please enter a valid system API endpoint.' };
    }

    if (!this.config.apiKey && !this.config.username) {
      this.connectionStatus = 'FAILED';
      this.log('ERROR', 'TEST', 'Authentication failed: Missing credentials.');
      return { success: false, latency: '-', message: 'Authentication failed: Missing API credentials or access token.' };
    }
    
    // Direct browser sandbox check:
    // Browser sandboxes cannot establish direct TCP/CORS connections to corporate networks without an integration agent.
    this.connectionStatus = 'EXTERNAL CONNECTOR REQUIRED';
    this.latency = '- (Gateway Required)';
    this.freshness = 'Unknown';
    this.lastSuccess = undefined;

    this.log('WARN', 'TEST', 'Configuration parameters saved. Direct browser connection prohibited by sandbox security; requires Orion Enterprise Gateway agent.');
    return { 
      success: false, 
      latency: '- (Gateway Required)', 
      message: 'Configuration validated. Connecting to this external enterprise system requires an active Orion Integration Agent or VPN Gateway.' 
    };
  }
  ...
  async sync(): Promise<SyncJobRecord> {
    ...
    if (this.type === 'FILE') {
      ...
      this.log('SUCCESS', 'SYNC', `Local file staging verified. ${this.records} staged records currently maintained.`);
      EventBus.emit('DATA_SYNCED', { connectorId: this.id, records: this.records });
      return job;
    }

    // For external enterprise systems without live backend agent:
    const now = new Date().toISOString();
    const job: SyncJobRecord = {
      id: jobId,
      source: this.name,
      ...
      recordsUpdated: 0,
      recordsRejected: 0,
      errors: ['External enterprise gateway agent required for remote polling. Direct browser TCP connection unavailable.'],
      status: 'WARNING'
    };
    ...
    this.records = 0;
    this.syncHistory.unshift(job);
    this.log('WARN', 'SYNC', `Sync halted: External backend connector agent required to poll remote endpoint.`);
    return job;
  }
...
// Event Bus for Orion SCM OS internal system updates
...
export const createDefaultConnectors = (): BaseConnector[] => {
  return [
    ...
    // File Importer (Pre-connected local parser)
    (() => {
      const fileConn = new BaseConnector({ id: 'files', name: 'File Importer', category: 'Data & Storage', description: 'CSV/Excel local file staging & ingestion pipeline', type: 'FILE', domains: ['Products', 'Inventory', 'Suppliers', 'Purchase Orders', 'Shipments'] });
      fileConn.connectionStatus = 'CONNECTED';
      fileConn.freshness = 'Fresh';
      fileConn.latency = '< 1ms (Local)';
      fileConn.lastSync = new Date().toISOString();
      fileConn.records = 0;
      fileConn.domains.forEach(d => { d.enabled = true; d.availableRecords = 0; });
      return fileConn;
    })()
  ];
};
