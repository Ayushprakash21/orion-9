...
import { Filter, Truck, AlertTriangle, Navigation, MapPin } from 'lucide-react';
...
export const Shipments = () => {
  ...
  const customsCount = activeShipments.filter(s => s.status === 'Customs').length;
  
  const totalValue = activeShipments.reduce((sum, s) => sum + (s.estimatedValue || 0), 0);
  const atRiskValue = activeShipments.filter(s => s.status === 'Delayed').reduce((sum, s) => sum + (s.estimatedValue || 0), 0);
  ...
  return (
    <div className="px-4 sm:px-6 md:px-8 py-6 w-full space-y-6 box-border">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <KPICard label="In Customs" value={customsCount} status={customsCount > 0 ? "warning" : "neutral"} />
        <KPICard label="Value in Transit" value={formatCurrency(totalValue, currency)} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div key={shipment.id} className="mb-6 last:mb-0">
                  ...
                  <div className="relative w-full h-1.5 bg-os-surface-secondary border border-os-border rounded-full overflow-hidden">
                    ...
                    <div 
                      ...
                      style={{
                        width: shipment.status === 'Preparing' ? '10%' :
                               shipment.status === 'In Transit' || shipment.status === 'Delayed' ? '50%' :
                               shipment.status === 'Customs' ? '75%' :
                               shipment.status === 'Out for Delivery' ? '90%' : '100%'
                      }}
                    />
                  </div>
                  <div className="flex justify-between text-[10px] text-os-text-muted mt-1">
                    <span>Dep: {safeFormatDate(shipment.departureDate, 'dd MMM', timezone)}</span>
                    <span>ETA: {safeFormatDate(shipment.estimatedArrival, 'dd MMM', timezone)}</span>
                  </div>
                </div>
              ))}
        ...
        <div className="bg-os-surface border border-os-border rounded-xl p-5 hover:border-os-border-strong transition-colors flex flex-col justify-between">
          ...
          {delayedCount > 0 ? (() => {
            const topRisk = activeShipments.filter(i => i.status === 'Delayed').sort((a,b) => b.delayDays - a.delayDays)[0];
            return (
              <div className="mt-2">
                <div className="text-lg font-mono text-os-text-primary">{topRisk.id}</div>
                <div className="text-xs text-os-text-secondary truncate">{topRisk.carrier} • {topRisk.method}</div>
                ...
                <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-md">
                  ...
                  <div className="text-[10px] text-os-text-muted mt-1">New ETA: {safeFormatDate(topRisk.estimatedArrival, 'dd MMM yyyy', timezone)}</div>
                </div>
              ...
              statusNode={
                <StatusBadge 
                  ...
                  type={
                    ...
                    (item.status === 'Customs' || item.status === 'Preparing') ? 'warning' : 'info'
                  } 
                />
              }
              fields={[
                { label: 'Carrier', value: item.carrier },
                { label: 'ETA', value: safeFormatDate(item.estimatedArrival, 'dd MMM yyyy', timezone), valueClassName: 'font-mono' },
              ...
              {filteredShipments.slice(0, 50).map((item) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-[10px] text-os-text-muted mt-0.5">{item.carrier} ({item.method})</div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap font-mono">
                    {safeFormatDate(item.departureDate, 'dd MMM yyyy', timezone)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap font-mono">
                    {safeFormatDate(item.estimatedArrival, 'dd MMM yyyy', timezone)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap font-mono">
                    {item.delayDays > 0 ? (
