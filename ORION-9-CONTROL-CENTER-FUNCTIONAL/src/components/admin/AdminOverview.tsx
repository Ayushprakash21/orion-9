...
import { Users, Building2, Activity, ArrowUpRight, BrainCircuit, ArrowRight, SlidersHorizontal } from 'lucide-react';
...
export const AdminOverview = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="p-5 rounded-lg border border-cyan-900/40 bg-cyan-950/10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-start gap-3.5">
          <div className="p-2.5 rounded-lg bg-cyan-600/10 text-cyan-400 border border-cyan-800/40 shrink-0 mt-0.5"><SlidersHorizontal size={22} /></div>
          <div><div className="flex items-center gap-2 mb-1"><h3 className="text-sm font-semibold text-os-text-primary">AI + Manual Control Center</h3><span className="text-[10px] font-mono px-2 py-0.2 rounded bg-cyan-900/30 text-cyan-400 border border-cyan-800/50">CONTROL PLANE</span></div><p className="text-xs text-os-text-secondary max-w-2xl leading-relaxed">Domain → Capability → Policy → Manual / AI Copilot / AI Autopilot → Governance → Audit.</p></div>
        </div>
        <NavLink to="/admin/control-center" className="px-3.5 py-1.5 text-xs font-medium rounded-md bg-cyan-600/15 hover:bg-cyan-600/25 text-cyan-300 border border-cyan-800/50 transition-colors shrink-0 flex items-center gap-1.5"><span>Open Control Center</span><ArrowRight size={13}/></NavLink>
      </div>
