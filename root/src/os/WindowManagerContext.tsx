...
export const WORKSPACES: WorkspaceConfig[] = [
  ...
  },
  {
    id: 'control',
    name: 'CONTROL',
    pinnedApps: ['exceptions', 'autonomous-operations', 'decisions', 'action-center', 'workflows', 'autopilot', 'attention-center', 'outcomes', 'decision-replay', 'policies']
  }
];
...
export function OrionWindowManager({ children }: { children: React.ReactNode }) {
  ...
  const computeDefaultGeometry = useCallback((existingCount: number) => {
    ...
    // On small mobile screens, default geometry is full-screen inside desktop stage
    const isMobile = screenW < 768;
    if (isMobile) {
      return {
        position: { x: 0, y: 0 },
        size: { width: screenW, height: Math.max(300, screenH - 32) }
      };
    }

    const stagger = (existingCount % 6) * 28;
    const width = Math.min(1140, Math.max(780, screenW - 160));
    const height = Math.min(740, Math.max(520, screenH - 180));
    const x = Math.max(20, Math.min(screenW - width - 20, Math.round((screenW - width) / 2) + stagger - 60));
    const y = Math.max(16, Math.min(screenH - height - 120, 24 + stagger));
  ...
  const resizeApplication = useCallback((
    ...
    setWindows(prev => {
      ...
      const minW = 480;
      const minH = 340;
      const clampedW = Math.max(minW, Math.min(screenW, size.width));
      const clampedH = Math.max(minH, Math.min(screenH - 32, size.height));
